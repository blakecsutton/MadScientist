...
class FacetedColumnCheckboxSelectMultiple(CheckboxSelectMultiple):
    ...
    def render(self, name, value, attrs=None, choices=()):
        ...
        facets = Facet.objects.annotate(size=Count('tag')).filter(**{'size__gt': 0}).values('name', 'size')
        unfaceted = Tag.objects.filter(facet=None).count() 
        ...
        style = u"<span style='width: 150px; float: left; padding: 10px;'><h3>%s</h3>"
        facet_index = 0
        if facet_index < len(facet_list) :
            
            output.append(style % facet_list[facet_index]['name'])
            facet_index += 1
